...
Experiment::Experiment(int numClients,
...
Experiment::Wifi(NodeContainer& c, std::map<int, std::shared_ptr<ClientSession>>& clients)
{
    ...
    Ptr<UniformRandomVariable> expVar =
        CreateObjectWithAttributes<UniformRandomVariable>("Min",
                                                          DoubleValue(m_txGain),
                                                          "Max",
                                                          DoubleValue(m_txGain + 30));
    ...
    // wifiChannel.SetPropagationDelay("ns3::RandomPropagationDelayModel", "Variable", StringValue
    // ("ns3::UniformRandomVariable[Min=0|Max=2]"));
    ...
    for (int j = 1; j <= numClients; j++)
    {
        ...
        {
            Experiment::SetPosition(c.Get(j),
                                    clients[j - 1]->GetRadius(),
                        ...
                        ns3::Time& timeOffset)
{
    ...
    sinkApps.Get(0)->GetObject<ns3::Server>()->SetClientSessionManager(&client_session_manager,
                                                                       m_flSymProvider,
                                                                       m_fp,
                                                                       m_round);
    ...
    if (!m_bAsync)
    {
        ...
        for (auto itr : stats)
        {
            ...
            NS_LOG_UNCOND("ID " << id << "  ,ADDRESS: " << itr.first
                                << " ,Round 1 Latency=" << itr.second.roundTime
                                << "s ,Round 1 Throughput= " << itr.second.throughput << "kbps");
